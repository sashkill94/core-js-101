...
function getPolynom(...args) {
  return (x) => {
    const res = args.reduce((acc, value, i) => acc + value * x ** (args.length - 1 - i), 0);
    return res;
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    logFunc(`${func.name}(${args.map((el) => JSON.stringify(el)).join(',')}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${args.map((el) => JSON.stringify(el)).join(',')}) ends`);
    return result;
  };
}
